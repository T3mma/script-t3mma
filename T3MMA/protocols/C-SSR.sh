...
Set_config_user() {
	...
	msg -bar
	stty erase '^H' && read -p "(Default: T3MMA):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="T3MMA"
...
Set_config_password() {
	...
	msg -bar
	stty erase '^H' && read -p "(Default: T3MMA):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="T3MMA"
